...
public abstract class SPARQL12UpdateComplianceTest extends SPARQLComplianceTest {
	...
	public SPARQL12UpdateComplianceTest() {
	...
	protected class DynamicSPARQL12UpdateComplianceTest extends DynamicSparqlComplianceTest {
		...
		public DynamicSPARQL12UpdateComplianceTest(String displayName, String testURI, String name, String requestFile,
	...
	public Collection<DynamicTest> getTestData() {
		...
		manifests.add(SPARQL12UpdateComplianceTest.class.getClassLoader()
				.getResource(testsSource)
				.toExternalForm());
		while (!manifests.isEmpty()) {
			String pop = manifests.pop();
			SPARQL12UpdateComplianceTest.SPARQLUpdateTestManifest manifest = new SPARQL12UpdateComplianceTest.SPARQLUpdateTestManifest(
					pop);
				...
				try (TupleQueryResult result = connection.prepareTupleQuery(query.toString()).evaluate()) {
					...
					for (BindingSet bs : result) {
						...
						String displayName = filename
								.substring(filename.lastIndexOf(testsSource)
										+ testsSource.length(), filename.lastIndexOf("/"))
								+ ": " + testName;
						...
						SPARQL12UpdateComplianceTest.DynamicSPARQL12UpdateComplianceTest ds12ut = new SPARQL12UpdateComplianceTest.DynamicSPARQL12UpdateComplianceTest(
						...
						if (!shouldIgnoredTest(testName)) {
							tests.add(DynamicTest.dynamicTest(displayName, ds12ut::test));
						}
					}
				}
			}
