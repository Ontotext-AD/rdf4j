...
/**
 * Test verifies the evaluation of TripleRef nodes in SPARQL 1.2 / RDF 1.2 context
 * with two different triple term storage strategies.
 *
 * <h2>Native Triple Term Storage (nativeSupport=true)</h2>
 * <p>Store implements {@link NativeTripleTermSource} and stores triple terms as first-class values.
 * Triple terms {@code <<( s p o )>>} are stored and queried directly without encoding.</p>
 *
 * <pre>
 ...
 * <p>Store does NOT support native triple terms and uses RDF 1.2 "basic encoding".
 * Each triple term is encoded using a proposition form resource with RDF vocabulary:</p>
 *
 * <pre>
 * // Input triple term:
 * <<( :s :p :o )>>
 ...
 * </pre>
 *
 * <p>When reification is involved (e.g., {@code << :s :p :o >> :q "value"}),
 * a reifier resource links to the proposition form:</p>
 *
 * <pre>
 ...
 * <p>This test focuses on TripleRef evaluation with proposition form encoding.
 * The proposition form resource is what gets bound to the TripleRef's extern variable
 * during evaluation when native support is not available.</p>
 *
 * <p>Both storage strategies should produce equivalent query results.</p>
 *
...
public class EvaluationStrategyWithTripleTermsTest {
	 ...
	 *
	 * <p><b>Native mode (nativeSupport=true):</b><br>
	 * Key = triple term itself (acts as its own identifier)<br>
	 * Value = the same triple term<br>
	 * The triple term is bound directly to the extern variable.</p>
	 *
	 * <p><b>Encoded mode (nativeSupport=false):</b><br>
	 * Key = proposition form resource (e.g., _:propForm or urn:1)<br>
	 * Value = the triple term it encodes ({@code <<( s p o )>>})<br>
	 * The proposition form resource is bound to the extern variable.</p>
	 */
	 ...
	 *
	 * <p><b>Native mode:</b> Returns triple terms directly via {@link #getTripleTerms(Resource, IRI, Value)}</p>
	 * <p><b>Encoded mode:</b> Returns proposition form statements via {@link #getStatements(Resource, IRI, Value, Resource...)}</p>
	 *
	 * @author damyan.ognyanov
	 */
	class CommonBaseSource {
		 ...
		 *
		 * <p>This method is called by {@link NativeTripleTermSource#getTripleTerms(Resource, IRI, Value)}
		 * when the store natively supports triple terms as first-class values.</p>
		 *
		 ...
		 *
		 * <p>In encoded mode, proposition forms represent triple terms that cannot be
		 * stored natively. This method returns the proposition form resources that
		 * will be queried for their RDF 1.2 encoding statements.</p>
		 *
		 ...
		 *
		 * <p>Implements the RDF 1.2 basic encoding vocabulary for triple terms.
		 * When a store cannot natively handle triple terms {@code <<( s p o )>>}, it encodes
		 * them using these RDF statements:</p>
		 *
		 * <ul>
		 ...
		 * </ul>
		 *
		 * <p>The evaluation strategy queries these statements to reconstruct triple terms
		...
		public CloseableIteration<? extends Statement> getStatements(Resource subj,
			...
			if (pred != null && pred.equals(RDF.TYPE) && obj != null && obj.equals(RDF.PROPOSITION_FORM)) {
				...
				return new ConvertingIteration<Resource, Statement>(
					...
					protected Statement convert(Resource sourceObject)
							throws QueryEvaluationException {
						return vf.createStatement(sourceObject, RDF.PROPOSITION_FORM_SUBJECT, triples.get(sourceObject).getSubject());
					}
				};
				...
				return new ConvertingIteration<Resource, Statement>(
					...
					protected Statement convert(Resource sourceObject)
							throws QueryEvaluationException {
						return vf.createStatement(sourceObject, RDF.PROPOSITION_FORM_PREDICATE, triples.get(sourceObject).getPredicate());
					}
				};
				...
				return new ConvertingIteration<Resource, Statement>(
					...
					protected Statement convert(Resource sourceObject)
							throws QueryEvaluationException {
						return vf.createStatement(sourceObject, RDF.PROPOSITION_FORM_OBJECT, triples.get(sourceObject).getObject());
					}
				};
			}
	...
	public void setUp() {
		...
		triples.clear();
		triples.put(vf.createIRI("urn:1"), vf.createTripleTerm(vf.createIRI("urn:a"), vf.createIRI("urn:p"), vf.createIRI("urn:b")));
		triples.put(vf.createIRI("urn:2"), vf.createTripleTerm(vf.createIRI("urn:a"), vf.createIRI("urn:q"), vf.createIRI("urn:b:1")));
		triples.put(vf.createIRI("urn:3"), vf.createTripleTerm(vf.createIRI("urn:a:1"), vf.createIRI("urn:p"), vf.createIRI("urn:b")));
		triples.put(vf.createIRI("urn:4"), vf.createTripleTerm(vf.createIRI("urn:a:2"), vf.createIRI("urn:q"), vf.createIRI("urn:c")));
	...
	public void testExtVarBound(boolean nativeSupport) {
		...
		for (Map.Entry<Resource, TripleTerm> entry : triples.entrySet()) {
			QueryBindingSet set = (QueryBindingSet) createWithVarValue(tripleRefNode.getExprVar(), nativeSupport ? entry.getValue() : entry.getKey());

			EvaluationStrategy strategy = new StrictEvaluationStrategy(createSource(nativeSupport), null);
			try (CloseableIteration<BindingSet> iter = strategy.precompile(tripleRefNode).evaluate(set)) {

				ArrayList<BindingSet> expected = new ArrayList<>();
				expected.add(nativeSupport ? fromTriple(entry.getValue()) : fromTriple(entry.getKey(), entry.getValue()));
				ArrayList<BindingSet> received = new ArrayList<>();
				while (iter.hasNext()) {
					received.add(iter.next());
				}
	 ...
	 *
	 * <p>In native mode, the extern variable is bound to the triple term itself,
	 * since triple terms are stored as first-class values.</p>
	 *
	 ...
	 *
	 * <p>In encoded mode, the extern variable is bound to the proposition form resource
	 * that encodes the triple term using RDF 1.2 basic encoding vocabulary.</p>
	 *
